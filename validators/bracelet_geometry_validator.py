...
功能：
1. 靜態檢查 src/domain/types.ts：確保 BraceletPlacement 零 px 污染、BeadSpec 乾淨收斂（不含價格）。
...
def validate():
    ...
    if not types_file.is_file():
        ...
        if "priceMinor" in content:
            errors.append("Domain Model Violation: 'priceMinor' was not removed from domain/types.ts (pricing is out of scope).")
    ...
    print("[bracelet_geometry_validator] PASS: Domain model boundary is clean (zero px, zero price) and invariant tests passed.")
    return 0
